...
const dbGetEvents = async ({ page = 1, limit = 10, isFeatured = false, sortByField = 'initialDate' } = {}) => {
    ...
    const [events, totalEvents] = await Promise.all([
        EventModel.find(filter)
            .sort({ [sortByField]: -1 }) // Ordenado por la fecha propia del evento
            .skip(skip)
            .limit(limit)
            .populate('bar'),
...
export { 
    // dbCreateEvent,
     dbGetEvents, dbGetEventById, dbGetEventsByBar, dbDeleteEvent, dbUpdateEvent };